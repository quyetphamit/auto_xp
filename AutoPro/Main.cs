...
namespace AutoSmart
{
    public partial class Main : Form
    {
        ...
        private string path = Path.Combine(Application.StartupPath, "data.txt");
        ...
        public void Run()
        {
            ...
            foreach (var item in _scripts)
            {
                ...
                {
                    var pos = item.Split(',');
                    LeftMouseClick(Convert.ToInt32(pos[0]), Convert.ToInt32(pos[1]));
                }
                else
                {
                    SendKeys.SendWait(txtBarcode.Text.Trim() + ".rpf");
                }
        ...
        private void btnGet_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            var data = string.Join(",", new string[] { MousePosition.X.ToString(), MousePosition.Y.ToString() });
            AutoPro.Common.SaveData(path, data);
            _scripts.Add(data);
            txtCountRecord.Text = _scripts.Count.ToString();
        }
        ...
        private void chkSendKey_CheckedChanged(object sender, EventArgs e)
        {
            if (chkSendKey.Checked)
            {
                ...
                {
                    _scripts.Add("SendKey");
                    txtCountRecord.Text = _scripts.Count.ToString();
                    //_query = "INSERT INTO tb_Scripts values ('SenKey')";
                    //_factory.ExecuteNonQuery(_query);
                    AutoPro.Common.SaveData(path, "SendKey");
                }
            }
        }
